...
def main():
    ...
    sized    = False   # window resized to the frame's pixel size (1:1)
    ...
    while True:
        ...
        if not sized:
            # Size the window to the frame once, so the image shows 1:1
            # (WINDOW_NORMAL alone opens at an arbitrary size and scales).
