...
Dalam drone-seeker, error tracking dikirim sebagai pesan `TRACKING_MESSAGE` (MAVLink ID 11045), pesan kustom yang didefinisikan dalam dialek ardupilotmega dan terdaftar dalam tabel `MAVLINK_MESSAGE_CRCS` yang dikompilasi ArduPlane. Pilihan ini didorong oleh mode kegagalan konkret: `mavlink_get_msg_entry()` MAVLink secara diam-diam membuang pesan apa pun yang ID-nya tidak ada dalam tabel yang dikompilasi, terlepas dari kontennya. Pesan yang dikirim dengan ID non-standar (ID 229, 230, 202 semuanya dicoba) dibuang di penerima sebelum mencapai lapisan aplikasi. Mendaftarkan `TRACKING_MESSAGE` sebagai pesan kustom dalam dialek ardupilotmega menyelesaikan masalah ini dan memungkinkan penerimaan error tracking yang andal dalam handler mode penerbangan `ModeTracking`.
